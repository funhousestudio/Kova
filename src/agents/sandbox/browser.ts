      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `kova sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `kova sandbox recreate --browser --agent ${agentId}`;
          }
          return "kova sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
