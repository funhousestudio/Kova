...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "kova.sandbox=1");
  args.push("--label", `kova.sessionKey=${params.scopeKey}`);
  args.push("--label", `kova.createdAtMs=${createdAtMs}`);
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`kova sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`kova sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("kova sandbox recreate --all");
}
