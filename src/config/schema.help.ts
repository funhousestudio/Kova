...
export const FIELD_HELP: Record<string, string> = {
  ...
  "audio.transcription.command":
    'Executable + args used to transcribe audio (first token must be a safe binary/path), for example `["whisper-cli", "--model", "small", "{{MediaPath}}"]`. Deprecated `{input}` placeholders are migrated to `{{MediaPath}}` by `kova doctor --fix`.',
  ...
  "session.maintenance.rotateBytes":
    'Deprecated and ignored. Do not use for `sessions.json` growth control; OpenClaw no longer creates automatic rotation backups, and "kova doctor --fix" removes this key.',
  ...
  "cron.webhook":
    'Deprecated legacy fallback webhook URL used by `kova doctor --fix` to migrate old jobs with `notify=true`. Runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.',
  "cron.webhookToken":
