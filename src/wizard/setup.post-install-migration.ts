...
function logMigrationHint(runtime: RuntimeEnv, candidate: ResolvedProviderCandidate): void {
  const command = formatCliCommand(`kova migrate ${candidate.provider.id} --dry-run`);
 ...
 * prompt) is owned by `kova migrate <provider>`; this helper only owns
 * the gate prompt.
 */
    ...
    try {
      ...
      params.runtime.log(
        `${candidate.provider.label} migration failed: ${formatErrorMessage(error)}. ` +
          `Re-run with ${formatCliCommand(`kova migrate ${candidate.provider.id} --dry-run`)} to inspect.`,
      );
    } finally {
