...
function resolveRequiredDeviceRole(
  ...
  defaultRuntime.error(
    `--device and --role are required. Run ${formatCliCommand("kova devices list")} to choose a paired device.`,
  );
...
export async function runDevicesRemoveCommand(
  ...
  if (!trimmed) {
    defaultRuntime.error(
      `deviceId is required. Run ${formatCliCommand("kova devices list")} to choose a paired device.`,
    );
    defaultRuntime.exit(1);
    return;
  }
