// `kova plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `kova plugins inspect`. */
...
export async function runPluginsInspectCommand(
  ...
  if (!inspect) {
    defaultRuntime.error(
      formatMissingPluginMessage({ id, listCommand: "kova plugins list --json" }),
    );
    return defaultRuntime.exit(1);
  }
  ...
  lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "kova"}`);
