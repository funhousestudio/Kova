...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          ["kova sessions --limit 25", "Show the newest 25 sessions."],
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          [
            "kova sessions cleanup --dry-run --fix-dm-scope",
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            'kova sessions compact "agent:main:main"',
            "LLM-summarize a session to reclaim context budget.",
          ],
          [
            'kova sessions compact "agent:main:main" --max-lines 200',
            "Truncate to the last 200 transcript lines instead.",
          ],
          [
            'kova sessions compact "agent:work:main" --agent work --json',
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `kova sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kova commitments", "List pending inferred follow-ups."],
          ["kova commitments --all", "List all inferred follow-ups."],
          ["kova commitments --agent work", "List one agent's inferred follow-ups."],
          ["kova commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
