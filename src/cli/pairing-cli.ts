...
function parseChannel(raw: unknown, channels: PairingChannel[]): PairingChannel {
  ...
  if (!value) {
    throw new Error(
      `Missing channel. Use ${formatCliCommand("kova pairing list --channel <channel>")}.`,
    );
  }
...
export function registerPairingCli(program: Command) {
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!approved) {
        throw new Error(
          `No pending pairing request found for code "${String(resolvedCode)}". Run ${formatCliCommand(`kova pairing list --channel ${channel}`)} to list pending requests.`,
        );
      }
