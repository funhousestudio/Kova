// `kova plugins list`: builds registry reports and defers terminal-only formatting modules.
...
export async function runPluginsListCommand(
  ...
  if (list.length === 0) {
    runtime.log(
      theme.muted(
        `No plugins found. Run ${formatCliCommand("kova plugins install <plugin>")} to add one, or ${formatCliCommand("kova plugins list --json")} to inspect raw discovery state.`,
      ),
    );
    return;
  }
