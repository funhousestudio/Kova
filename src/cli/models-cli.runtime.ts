...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `kova models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("kova agents list")} and set the per-agent model in agent config.`,
  );
}
