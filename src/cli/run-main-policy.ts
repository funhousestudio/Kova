...
export function resolveMissingPluginCommandMessage(
  ...
  if (parentPluginId) {
    ...
    if (
      ...
      return (
        `The \`kova ${normalizedPluginId}\` command is provided by the ` +
        `"${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run ` +
        `\`kova plugins enable ${parentPluginId}\` to enable that CLI surface.`
      );
    }
    ...
    if (ownerEnabled) {
      ...
      if (toolOwner.availability === "manifest-only") {
        return (
          ...
          "Run `kova --help` to see available CLI subcommands."
        );
      }
      return (
