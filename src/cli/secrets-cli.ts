...
async function readPlanFile(pathname: string): Promise<SecretsApplyPlan> {
  ...
  if (!isSecretsApplyPlan(parsed)) {
    throw new Error(
      `Invalid secrets plan file: ${pathname}. Generate a fresh plan with ${formatCliCommand("kova secrets configure --plan-out <path>")}.`,
    );
  }
  return parsed;
}
      ...
      defaultRuntime.error(
        danger(
          formatGatewayCommandFailure({
            ...
            error: err,
            inspectCommand: "kova gateway status --deep",
          }),
        ),
      );
  ...
  secrets
    ...
    .action(async (opts: SecretsAuditOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets audit failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("kova doctor")} to inspect config and credential state.`,
          ),
        );
  ...
  secrets
    ...
    .action(async (opts: SecretsConfigureOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets configure failed: ${formatErrorMessage(err)}. Re-run ${formatCliCommand("kova secrets audit")} before applying changes.`,
          ),
        );
        defaultRuntime.exit(1);
      }
  ...
  secrets
    ...
    .action(async (opts: SecretsApplyOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets apply failed: ${formatErrorMessage(err)}. Re-run ${formatCliCommand("kova secrets apply --from <path> --dry-run")} to inspect the plan without writing.`,
          ),
        );
        defaultRuntime.exit(1);
      }
