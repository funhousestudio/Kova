...
function collectPlaintextConfigSecretWarnings(cfg: OpenClawConfig): string[] {
  ...
  return [
    ...
    `  Migrate them to SecretRefs with ${formatCliCommand("kova secrets configure")} or ${formatCliCommand("kova secrets apply")}, then verify with ${formatCliCommand("kova secrets audit --check")}.`,
  ];
}
