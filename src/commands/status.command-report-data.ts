// Builds the data model for the standard `kova status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("kova security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("kova status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("kova tasks maintenance --apply")}`,
