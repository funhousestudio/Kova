...
export async function maybeMigrateAuthProfileJsonStoresToSqlite(params: {
  ...
  note(
    [
      ...
      `- ${formatCliCommand("kova doctor --fix")} imports legacy auth profile JSON into the per-agent SQLite database and removes the old files after backup.`,
    ].join("\n"),
    ...
    try {
      ...
      if (unresolvedSidecarProfileIds.size > 0) {
        ...
        result.warnings.push(
          `Left ${unresolvedSidecarProfileIds.size} legacy OAuth sidecar profile${unresolvedSidecarProfileIds.size === 1 ? "" : "s"} in ${shortenHomePath(candidate.authPath)}; rerun ${formatCliCommand("kova doctor --fix")} after sidecar migration or re-authenticate those profiles.`,
        );
      }
...
export async function maybeRepairLegacyFlatAuthProfileStores(params: {
  ...
  if (legacyStores.length > 0) {
    noteLines.push(
      `- The gateway expects the canonical version/profiles store; ${formatCliCommand("kova doctor --fix")} rewrites this legacy shape with a backup.`,
    );
  }
  if (awsSdkMarkerStores.length > 0) {
    noteLines.push(
      `- AWS SDK profile markers are routing metadata, not stored credentials; ${formatCliCommand("kova doctor --fix")} moves them to config with a backup.`,
    );
  }
  note(noteLines.join("\n"), "Auth profiles");
...
export async function maybeRepairCanonicalApiKeyFieldAlias(params: {
  ...
  noteLines.push(
    `- Runtime auth parsing only reads canonical "key" and "keyRef" fields, so these profiles are silently skipped; ${formatCliCommand("kova doctor --fix")} rewrites "api_key" to "key" with a backup.`,
  );
  note(noteLines.join("\n"), "Auth profiles");
