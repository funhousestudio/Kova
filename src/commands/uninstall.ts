...
async function stopAndUninstallService(runtime: RuntimeEnv): Promise<boolean> {
  if (isNixMode) {
    ...
    runtime.error(
      `Nix mode detected; service uninstall is disabled. Manage the service through your Nix profile instead, then run ${formatCliCommand("kova status")} to verify.`,
    );
    return false;
  }
  ...
  try {
    ...
    runtime.error(
      `Gateway service check failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("kova gateway status --deep")} for service diagnostics.`,
    );
    return false;
  }
  if (!loaded) {
  ...
  try {
    ...
    runtime.error(
      `Gateway stop failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("kova gateway status --deep")} before retrying uninstall.`,
    );
  }
  try {
    ...
    runtime.error(
      `Gateway uninstall failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("kova gateway status --deep")} for the service state.`,
    );
    return false;
  }
}
...
export async function uninstallCommand(runtime: RuntimeEnv, opts: UninstallOptions) {
  ...
  if (!interactive && !opts.yes) {
    runtime.error(
      `Non-interactive uninstall requires --yes. Preview first with ${formatCliCommand("kova uninstall --dry-run --all")}.`,
    );
    runtime.exit(1);
    return;
  }
