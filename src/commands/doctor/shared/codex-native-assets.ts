...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      ...
      "- If the Codex plugin is not installed, run `kova plugins install npm:@openclaw/codex` first. Then run `kova migrate plan codex` to inventory them. Applying that migration copies skills into the current OpenClaw agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
