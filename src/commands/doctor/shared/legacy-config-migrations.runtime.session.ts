...
const LEGACY_SESSION_MAINTENANCE_ROTATE_BYTES_RULE: LegacyConfigRule = {
  ...
  message:
    'session.maintenance.rotateBytes is deprecated and ignored; run "kova doctor --fix" to remove it.',
...
const LEGACY_SESSION_PARENT_FORK_MAX_TOKENS_RULE: LegacyConfigRule = {
  path: ["session"],
  message:
    'session.parentForkMaxTokens was removed; parent fork sizing is automatic. Run "kova doctor --fix" to remove it.',
