...
// update.
//
// `runGatewayUpdate` (git mode) runs `kova doctor --fix` with
...
// `shouldDeferConfiguredPluginInstallRepair`). The `kova update` CLI resumes
...
// binary's hidden `kova update finalize` entrypoint — the designed
...
// re-enters the full `kova update` CLI. Re-run convergence on no-op retries:
