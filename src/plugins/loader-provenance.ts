...
function formatPluginInspectCommand(pluginId: string): string {
  return `kova plugins inspect ${quoteCliArg(pluginId)}`;
}
...
export function warnWhenAllowlistIsOpen(params: {
  ...
  const remediation = truncated
    ? "Run 'kova plugins list --enabled --verbose' to enumerate every discovered plugin id, inspect trusted ids with 'kova plugins inspect <id>', and add the ones you trust to plugins.allow in openclaw.json."
    : `To trust them explicitly, set plugins.allow in openclaw.json (e.g. "plugins": { "allow": [${autoDiscoverable
        ...
        .join(
          ", ",
        )}] }). Run 'kova plugins list --enabled --verbose' or ${inspectCommands} to confirm plugin ids.`;
