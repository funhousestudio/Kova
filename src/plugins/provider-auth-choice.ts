...
export async function applyAuthChoicePluginProvider(
  ...
  if (!provider) {
    await params.prompter.note(
      `${options.label} auth plugin is not available. Install or enable the plugin, then rerun onboarding. If this started after an update, run "kova doctor --fix" first.`,
      options.label,
    );
    return { config: nextConfig };
  }
